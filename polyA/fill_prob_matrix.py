...
def fill_prob_matrix(
    ...
    for row_index in range(row_count):
        origins[(row_index, 0)] = -1
        probabilities[(row_index, 0)] = 0.0

    # col_index is j in the Perl version
    col_index = 1
    ...
    # col_index_index is col in the Perl version
    for col_index_index in range(1, len(columns) + chunk_size - 1):
        ...
        for row_index in range(row_count):
            ...
            support = support_matrix.get((row_index, col_index), None)
            if support is not None and support != 0:
                support_log = math.log(support)

                for inner_row_index in range(row_count):
                    # Omitting check on line 841 because we expanded columns
                    if in_provided_columns:
                        prob_value = probabilities.get(
                            (inner_row_index, columns[col_index_index - 1]),
                            None,
                        )
                    else:
                        prob_value = probabilities.get(
                            (inner_row_index, col_index - 1), None
                        )

                    if prob_value is not None:
                        score = support_log + prob_value

                        if row_index == inner_row_index:
                        ...
                        if score > max_score:
                            max_score = score
                            max_score_index = inner_row_index

                probabilities[(row_index, col_index)] = max_score
                origins[(row_index, col_index)] = max_score_index
