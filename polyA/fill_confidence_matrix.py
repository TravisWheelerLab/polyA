...
def fill_confidence_matrix(
    ...
    >>> align_mat = {
    ...     (0, 0): 0,   (0, 1): 100, (0, 2): 99,
    ...     (1, 0): 100, (1, 1): 100, (1, 2): 100,
    ... }
    >>> active = {0: [0, 1], 1: [0, 1], 2: [0, 1]}
    >>> cols = [0, 1, 2]
    >>> counts = {"s1": 0.33, "s2": 0.33}
    >>> subs = ["s1", "s2"]
    >>> conf_mat = fill_confidence_matrix(cols, counts, subs, active, align_mat)
    >>> f"{conf_mat[0,0]:.4f}"
    '0.0100'
    >>> f"{conf_mat[1,0]:.4f}"
    '0.9900'
    ...
    for col_index in columns:
        temp_region = [
            align_matrix[row_index, col_index]
        ...
        for row_index in range(len(active_cells[col_index])):
            confidence_matrix[
                active_cells[col_index][row_index], col_index
            ] = temp_confidence[row_index]
