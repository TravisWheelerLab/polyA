...
def fill_node_confidence(nodes: int, start_all: int, gap_init: int, gap_ext: int, lamb: float, infilee: str,
    ...
    for subfam_index2 in range(1, len(subfams) - tr_count):
        ...
        for i in range(begin_node2, begin_node2 + columns[changes_position[-1] - 1] - columns[changes_position[-2]]):
        ...
        if end_node2 >= starts[subfam_index2] - start_all and begin_node0 <= stops[subfam_index2] - start_all:
            align_score2 = calculate_score(gap_ext, gap_init, subfam2, chrom2, lastprev_subfam2, lastprev_chrom2, sub_matrix)
    ...
    for node_index4 in range(nodes):
        ...
        confidence_temp: List[float] = confidence_cm(lamb, infilee, temp, subfam_countss, subfams, subfam_rows, tr_count)
