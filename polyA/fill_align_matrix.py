...
def fill_hmm_align_matrix(
    ...
    for i in range(1, len(chroms)):
        ...
        for k in range(half_chunk - 1, -1, -1):
            if (
                ...
                if subfams[i][seq_index + offset] == "-":  # next in subfam
                    ...
                    hmm_end += 1  # only move end if next char in subfam is not -
                ...
                while temp_count < chunk_size - k:
                    if chrom_seq[temp_index] != "-":
                        temp_count += 1
                    if subfam_seq[temp_index] == "-" or subfam_seq[temp_index] == ".":
                        hmm_offset += 1
                    temp_index += 1
        ...
        print(subfam_seq[seq_index: seq_index + offset])
        print(hmm_start)
        print(hmm_end)

        # should have hmm_end - hmm_start + 1 chars (not gaps)
        col_index += 1
        num_nucls: int = chunk_size  # how many nucls contributed to align score
        ...
        while seq_index + offset < len(chrom_seq):
            ...
            if chrom_seq[seq_index + 1] != "-":
                ...
                if (
                    ...
                    while temp_count < chunk_size:
                        if chrom_seq[temp_index + 1] != "-":
                            temp_count += 1
                        if subfam_seq[temp_index + 1] == "-" or subfam_seq[temp_index + 1] == ".":
                            hmm_offset += 1
                        temp_index += 1

                    offset = temp_index - seq_index
                    print("recalc")
                    hmm_end = hmm_start + offset - hmm_offset  # new hmm end
                    # FIXME - stop moving hmm end with padding
                    ...
                    if subfam_seq[seq_index] == "-":
                        ...
                        emission_score = float(subfam_hmm[str(hmm_start)]['emission'][chroms[i][seq_index]])
                        align_score = (
                            align_score
                            - emission_score
                        )
            ...
            print(hmm_start + 1)
            print(hmm_end)
            print(subfam_seq[seq_index + 1: seq_index + offset + 1])
            hmm_start += 1
            seq_index += 1
        exit()
        # add trailing cells and normalizes
        ...
        # FIXME: is this right?
        if subfam_seq[seq_index] != "-":
            hmm_start += 1  # only moves forward in hmm if no gap
        while chrom_seq[seq_index] == "-":
            if subfam_seq[seq_index] != "-":
                hmm_start += 1  # only moves forward in hmm if no gap
            seq_index += 1
