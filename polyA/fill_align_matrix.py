...
def fill_hmm_align_matrix(
    ...
    for i in range(1, len(chroms)):
        ...
        hmm_offset: int = 0  # num of moves forward in subfam seq
        while temp_count < chunk_size - k:
            if chrom_seq[temp_index] != "-":
                temp_count += 1
        ...
        start_gap_index: int = -1
        # FIXME: will alignment ever start with an insert? NO (or deletion)
        ...
        while seq_index + offset < len(chrom_seq):
            ...
            if subfam_seq[seq_index] == "-":
                ...
                if start_gap_index != seq_index:
                    start_insertion_score = calculate_insertion_score(
                        hmm_start,
                        chrom_seq[seq_index:],
                        subfam_seq[seq_index:],
                        subfam_hmm,
