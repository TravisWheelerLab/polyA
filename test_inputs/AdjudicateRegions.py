...
chunksize: int = 31
sameProbLog: float = log(
    1 - (10 ** -45)
)  # FIXME - this just becomes 0.0... need to be more precise
...
SubFamsCollapse: Dict[
    str, int
] = {}  # FIXME - this is just a dict with a string as the value
...
sameProbSkip = changeProbLog / 10
# 10% of the jump penalty, staying in skip state for 20nt "counts" as one jump
...
def padSeqs(start, stop, subfamseq, chromseq):
    ...
    for i in range(1, len(SubfamSeqs)):
        ...
        chromseq[i] = (
            ("." * leftpad) + f"{chromseq[i]}" + ("." * (rightpad + 15))
        )
        subfamseq[i] = (
            ("." * leftpad) + f"{subfamseq[i]}" + ("." * (rightpad + 15))
        )
...
def CalcScore(seq1: str, seq2: str, lastpreva: str, lastprevb: str):
    ...
    if seq1[0] == "-":
        ...
        chunkscore += SubMatrix[
            CharPos[seq1[0]] * subMatrixCols + CharPos[seq2[0]]
        ]
    ...
    for j in range(1, len(seq1)):
        if seq1[j] == "-":
            ...
            if seq1[j] in CharPos and seq2[j] in CharPos:
                chunkscore += SubMatrix[
                    CharPos[seq1[j]] * subMatrixCols + CharPos[seq2[j]]
                ]
...
def FillAlignScoreMatrix(subfams: List[str], chroms: List[str]):
    ...
    for i in range(1, len(ChromSeqs)):
        ...
        index = (
            j + 15
        )  # index is the col we are in the align score matrix, $j is the place in @subfam1 and @chrom1
        alignscore: int = 0
        tempindex: int = j
        tempcount: int = 0
        ...
        for k in range(15, -1, -1):
            ...
            AlignHash[i, index - k] = int(
                alignscore * chunksize / (chunksize - k)
            )  # already to scale so don't need to * 31 and / 31
        ...
        while j + offset < len(chrom1):
            ...
            if chrom1[j + 1] != "-":
                if chrom1[j + 1] != "." and subfam1[j + 1] != ".":
                    if (
                        prevoffset != offset
                    ):  # there is a new gap, or a gap was removed from beginning
                        ChromSlice = chrom1[j + 1 : j + offset + 1]
                        SubfamSlice = subfam1[j + 1 : j + offset + 1]
                        alignscore = CalcScore(
                            SubfamSlice, ChromSlice, subfam1[j], chrom1[j]
                        )
                        ...
                        if subfam1[j] == "-":
                            ...
                            alignscore = (
                                alignscore
                                - SubMatrix[
                                    CharPos[subfam1[j]] * subMatrixCols
                                    + CharPos[chrom1[j]]
                                ]
                            )
                            numnucls -= 1
                        ...
                        if (
                            ...
                            alignscore = (
                                alignscore
                                + SubMatrix[
                                    CharPos[subfam1[j + offset]] * subMatrixCols
                                    + CharPos[chrom1[j + offset]]
                                ]
                            )
                            numnucls += 1
                    ...
                    if alignscore <= 0:
                        ...
                        AlignHash[i, index] = int(
                            alignscore / numnucls * chunksize
                        )
...
FillConsensusPosMatrix(
    ConsensusHash, SubfamSeqs, ChromSeqs, ConsensusStarts, ConsensusStops
)
    ...
    for i in range(rows):
        for col in range(len(Columns)):
            ...
            if (i, j) in confhash:
                ...
                numsegments = 0
                while num < chunksize:
                    if (i, j + num) in confhash:
                        summ += confhash[i, j + num]
                        numsegments += 1
...
FillSupportMatrix(SupportHash, ConfHash)
...
def FillProbMatrix(
    probhash: Dict[Tuple[str, int], float],
    supporthash: Dict[Tuple[str, int], float],
    originhash: Dict[Tuple[str, int], str],
):
        ...
        for i in ActiveCellsCollapse[Columns[j]]:
            ...
            for row in ActiveCellsCollapse[Columns[j - 1]]:
                ...
                if row == i:  # staying in same row
                    ...
                    if (
                        StrandHashCollapse[row, Columns[j - 1]]
                        != StrandHashCollapse[i, Columns[j]]
                    ):
                        prob = changeProbLog
                    else:  # if on same strand
                        if StrandHashCollapse[row, Columns[j - 1]] == "+":
                            if (
                                ConsensusHashCollapse[row, Columns[j - 1]]
                                > ConsensusHashCollapse[i, Columns[j]] + 50
                            ):
                                prob = changeProbLog
                        if StrandHashCollapse[row, Columns[j - 1]] == "-":
                            if (
                                ConsensusHashCollapse[row, Columns[j - 1]] + 50
                                < ConsensusHashCollapse[i, Columns[j]]
                            ):
                                prob = changeProbLog
...
def NodeConfidence(
    ...
    nodeconfidence_temp: List[float] = [
        0 for _ in range(len(SubFams) * numnodes)
    ]
    ...
    for i in range(1, numnodes - 1):
        for j in range(1, len(SubFams)):
            ...
            subfam: str = subfamseqs[j][b:e]
            chrom: str = chromseqs[j][b:e]
            lastpreva: str = subfamseqs[j][
                b - 1
            ]  # subfamseqs[j][Columns[changespos[i + 1] - 1]]
            lastprevb: str = chromseqs[j][
                b - 1
            ]  # chromseqs[j][changespos[i + 1] - 1]
    ...
    for j in range(numnodes):
        ...
        confidenceTemp: List[float] = [
            float(x) for x in ConfidenceCM(lamb, temp).split(" ")
        ]
    ...
    for j in range(numnodes):
        for i in range(len(SubFams)):
            if (SubFams[i], j) in nodeconfidence:
                nodeconfidence[SubFams[i], j] += nodeconfidence_temp[
                    i * numnodes + j
                ]
            else:
                nodeconfidence[SubFams[i], j] = nodeconfidence_temp[
...
def FillPathGraph(pathgraph: List[int]):
    ...
    for j in range(numnodes):
        ...
        sinkSubfamStart: int = ConsensusHashCollapse[
            sinkSubfam, Columns[ChangesPos[j]]
        ]
        ...
        for i in range(j - 1):
            ...
            for sourceSubfam in SubFamsCollapse:
                ...
                if (
                    sourceSubfam,
                    Columns[ChangesPos[i + 1] - 1],
                ) in ConsensusHashCollapse:
                    sourceSubfamStop = ConsensusHashCollapse[
                        sourceSubfam, Columns[ChangesPos[i + 1] - 1]
                    ]
                    sourceStrand = StrandHashCollapse[
                        sourceSubfam, Columns[ChangesPos[i + 1] - 1]
                    ]
...
def ExtractNodes(
    removestarts, removestops, changespos, pathgraph, numnodes: int
):
    global cols
    ...
    while RemoveNodes[i]:
        cols = Columns[
            changespos[i] - 1
        ]  # FIXME - do I index cols here or not?
        i -= 1
...
def PrintResultsSequence():
    ...
    for i in range(len(Changes_orig)):
        if str(Changes_orig[i]) != "skip":
            ...
            stderr.write("\t")
            stderr.write(
                str(Columns_orig[ChangesPos_orig[i + 1] - 1] + startall)
            )
            stderr.write("\t")
            stderr.write(str(IDs[Columns_orig[ChangesPos_orig[i]]]))
            stderr.write("\t")
